...
export type StoredSyllabusListItem = {
  id: number;
  name: string;
  rating: string;
};
...
export type StoredSection = {
  id: number;
  name: string;
  rows: StoredContentRow[];
};

export type StoredSyllabusTree = {
  syllabus: StoredSyllabusListItem;
  sections: StoredSection[];
};

const ORDER_SQL = "ORDER BY FIELD(rating, 'ADM', 'C1', 'S3', 'S2', 'S1') DESC, id ASC";

export async function storedSyllabusEnabled(): Promise<boolean> {
...
export async function listStoredSyllabi(opts?: { includeAdmin?: boolean }): Promise<StoredSyllabusListItem[]> {
  const ok = await storedSyllabusEnabled();
  if (!ok) return [];
...
export async function getStoredSyllabusTree(syllabusId: number, controllerCid: number): Promise<StoredSyllabusTree | null> {
  const ok = await storedSyllabusEnabled();
  if (!ok) return null;
  ...
  const syllabus: StoredSyllabusListItem = {
  ...
  const entryByContent = new Map<number, StoredEntry>();
  ...
  const bySection = new Map<number, StoredContentRow[]>();
  ...
  const outSections: StoredSection[] = sections.map((s) => ({
    id: Number(s.id),
    name: String(s.name ?? ''),
...
export async function upsertStoredProgress(controllerCid: number, contentId: number, progress: number, observerCid: number): Promise<void> {
  const ok = await storedSyllabusEnabled();
  if (!ok) return;
...
export async function deleteStoredProgress(controllerCid: number, contentId: number): Promise<void> {
  const ok = await storedSyllabusEnabled();
  if (!ok) return;
  const cCid = Number(controllerCid);
  const cId = Number(contentId);
